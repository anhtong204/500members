...
(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.holateam = {}, global.jQuery));
	...
	*/
	...
	const initializeBlock = function (block) {
	  const settings = block.find('.holateam-slides').data('settings');
	  const defaults = {
	    dots: false,
	    speed: 300,
	    slidesToShow: 1,
	  ...
	  const slider = block.find('.holateam-slides').slick(Object.assign(defaults, settings));
	...
	(function ($) {
	  ...
	  function initListingSidebar($layout) {
	    ...
	    var isArchivePage = $layout.closest('.component-training').length === 0; // Filter cards by category slug
	    ...
	    function initAccordion($container) {
	      $container.find('.training-sidebar-lessons').hide(); // Open active category or first
	    ...
	    function bindCategoryClicks($container) {
	      $container.find('.training-sidebar-category-btn').on('click', function (e) {
	        ...
	        var isActive = $category.hasClass('active'); // Accordion: close all, toggle clicked
	        ...
	        if (!isArchivePage) {
	          ...
	          filterByCategory(slug); // Update URL without reload
	    ...
	    if ($mobileModal.length) {
	      ...
	      bindCategoryClicks($mobileModal); // Open modal
	  ...
	  function initDetailSidebar($layout) {
	    ...
	    function initDetailAccordion($container) {
	      $container.find('.training-sidebar-lessons').hide(); // Auto-open active category
	      ...
	      $container.find('.training-sidebar-category-btn').on('click', function (e) {
	        ...
	        var url = $link.attr('href');
	        var isActive = $category.hasClass('active'); // Toggle accordion
	    ...
	    markDoneInSidebar($layout); // Done button
	  ...
	  function saveCompletedTrainings(arr) {
	    ...
	    document.cookie = 'training_done=' + encodeURIComponent(JSON.stringify(arr)) + ';expires=' + expires.toUTCString() + ';path=/;SameSite=Lax';
	  }
	...
	(function ($) {
	  $(document).ready(function () {
	    if (!$('.survey-component-wrapper').length) return; // Open modal and show target step

	    $('.survey-btn-choice').on('click', function (e) {
	      e.preventDefault();
	      const target = $(this).data('target'); // Hide all steps in modal

	      $('.survey-step-content').hide(); // Show the target step

	      $('#' + target).fadeIn(300); // Show Modal
	    ...
	    attachHasInputListeners(); // Next button logic

	    $('.survey-btn-next').on('click', function (e) {
	      const nextTarget = $(this).data('next');
	      const currentStep = $(this).closest('.survey-step-content'); // HTML5 Validation for the current step
